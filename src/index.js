...
const createElement = (id, x1, y1, x2, y2, type, color,size) => {
  switch (type) {
    case "line":
    case "rectangle":
      const roughElement =
        type === "line"
          ? generator.line(x1, y1, x2, y2,{stroke:color, strokeWidth: size})
          : generator.rectangle(x1, y1, x2 - x1, y2 - y1,{fillStyle: 'solid',fill: color, strokeWidth:0,stroke: color});
...
const drawElement = (roughCanvas, context, element, size) => {
  switch (element.type) {
    ...
    case "rectangle":
      ...
      element.roughElement.fill=element.color
      ...
      break;
    case "pencil":
      const stroke = getSvgPathFromStroke(getStroke(element.points,{simulatePressure: false,size: size}));
...
const App = () => {
  ...
  const updateElement = (id, x1, y1, x2, y2, type, options) => {
    ...
    switch (type) {
      case "line":
      case "rectangle":
        elementsCopy[id] = createElement(id, x1, y1, x2, y2, type,color,size);
        break;
      case "pencil":
  ...
  return (
    <div>
      ...
      <Slider  value={size} onChange={(e,value) => {
            setSize(value);}} aria-label="Default" valueLabelDisplay="auto" />
      <ChromePicker disableAlpha={true} onChange={(color) => {
